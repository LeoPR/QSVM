...
def test_reconstruct_identity():
    ...
    extractor = OptimizedPatchExtractor(patch_size=(4,4), stride=2,
                                        cache_dir="./.cache_test", image_size=(H,W), max_memory_cache=2)
    patches = extractor.process(img, index=0)
    recon = extractor.reconstruct_image(patches)  # [H,W] uint8
    # tolerância pequena por sobreposição/round
    diff = (recon.numpy().astype(np.int32) - grid.astype(np.int32))
    mae = np.abs(diff).mean()
    assert mae <= 2.0  # bem próximo do original